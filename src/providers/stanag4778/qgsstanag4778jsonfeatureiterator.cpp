...
QgsStanag4778JsonFeatureIterator::QgsStanag4778JsonFeatureIterator(QgsStanag4778JsonFeatureSource *source, bool ownSource, const QgsFeatureRequest &request)
  : QgsAbstractFeatureIteratorFromSource<QgsStanag4778JsonFeatureSource>(source, ownSource, request) {
  ...
  } else mUsingFeatureIdList = false;

  rewind();
}

QgsStanag4778JsonFeatureIterator::~QgsStanag4778JsonFeatureIterator() {
  close();
}

bool QgsStanag4778JsonFeatureIterator::fetchFeature(QgsFeature &feature) {
  feature.setValid(false);

  if(mClosed) return false;

  if(mUsingFeatureIdList) return nextFeatureUsingList(feature);
  else return nextFeatureTraverseAll(feature);
}


bool QgsStanag4778JsonFeatureIterator::nextFeatureUsingList(QgsFeature &feature) {
...
bool QgsStanag4778JsonFeatureIterator::nextFeatureTraverseAll(QgsFeature &feature) {
  bool hasFeature = false;

  // option 2: traversing the whole layer
  while(mSelectIterator != mSource->mFeatures.constEnd()) {
    if (mFilterRect.isNull()) {
      // When the selection rect is empty, use all features
      hasFeature = true;
    }
    else
    {
...
bool QgsStanag4778JsonFeatureIterator::rewind() {
  if(mClosed) return false;

  if(mUsingFeatureIdList) mFeatureIdListIterator = mFeatureIdList.constBegin();
  else mSelectIterator = mSource->mFeatures.constBegin();

  return true;
}

bool QgsStanag4778JsonFeatureIterator::close() {
  if(mClosed) return false;
...
QgsStanag4778JsonFeatureSource::QgsStanag4778JsonFeatureSource(const QgsStanag4778JsonProvider *p)
  : mFields(p->mFields)
...
QgsFeatureIterator QgsStanag4778JsonFeatureSource::getFeatures(const QgsFeatureRequest &request) {
  return QgsFeatureIterator( new QgsStanag4778JsonFeatureIterator( this, false, request ) );
}
